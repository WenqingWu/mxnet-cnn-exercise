...
# 首先它需要拿到一个列表文件，格式为”下标\t标签\t文件路径”
# 之后它遍历这个列表的每一行，打开文件->预处理->保存到数据集的指定下标处->给定对应的标签
# 然后使用im2rec.py工具，第一个参数prefix是列表文件的路径（不含扩展名”.lst”），第二个参数是图片所在的目录。
# 笔者还用到了–resize参数，将图片的长边缩放至32像素。假如你的列表文件叫做list_folder/train.lst，图片位于data_folder/train目录，则运行
